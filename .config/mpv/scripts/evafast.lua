...
local function adjust_speed()
    ...
    if speedup_target ~= nil then
        ...
        if current_time >= speedup_target then
            ...
            local time_for_correction = speed_transition(speed,
                math.max(
                math.min(options.speed_cap, options.subs_speed_cap and options.subs_speed_cap or options.speed_cap), 1.1))                                                          -- not effective_speed_cap because it may lead to huge fluctuations in transition speed
